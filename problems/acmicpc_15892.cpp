...
class flow {
	...
	public:
		static pair<edge*, edge*> make_edge(vertex const *from, vertex const *to, int capacity) {
			...
			auto reverse = new edge(to, from, capacity);
		...
		[[nodiscard]] bool size() const { return capacity; }
	};
	class vertex {
	public:
		vector<edge*> edges;
	...
	int find_flow() {
		...
		while(!q.empty()) {
			...
			for(auto e : vertices[now].edges) {
				...
				if(next == 1 || visited[next] || e->full()) continue;
				q.push({ next, e, min<int>(in_flow, e->size()) });
			}
		} return 0;
	}
};
